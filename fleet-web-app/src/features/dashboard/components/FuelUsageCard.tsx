...
const FuelUsageCard = ({ fuelByDay, activeVehicles }: FuelUsageCardProps) => {
  ...
  const dayLabels = useMemo(() => {
    const now = new Date();
    return fuelByDay.map((_, index) => {
      const date = new Date(now);
      date.setDate(now.getDate() - (fuelByDay.length - 1 - index));
  ...
  return (
    <div className="learning-card p-5">
      <h2 className="text-sm font-black text-gray-800 dark:text-gray-200 mb-3">{t('dashboard.fuel.title')}</h2>
      ...
      <div className="flex justify-between text-[10px] text-gray-400 mb-3">
        {dayLabels.map((label, index) => (
          <span key={`fuel-day-label-${index}`}>{label}</span>
        ))}
      </div>
      ...
      <div className="flex justify-between text-sm py-1.5">
        ...
        <span className="text-gray-900 dark:text-white font-semibold">
          {t('dashboard.stats.tnd', { n: Math.round(cost).toLocaleString(locale) })}
        </span>
      </div>
    </div>
  );
