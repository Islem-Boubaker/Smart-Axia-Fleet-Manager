...
const FuelAnalysis = ({ fuelData, dark = false }: Props) => {
  ...
  return (
    <Card
      title={t('reports.fuelBreakdown.title')}
      subtitle={t('reports.fuelBreakdown.subtitle')}
      padding="lg"
      dark={dark}
      ...
      <div className="space-y-7">
        {fuelData.map((fuel) => (
          <div key={fuel.type} className="space-y-3">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-2 flex-wrap min-w-0">
                ...
                <span className={`text-sm ${dark ? 'text-slate-400' : 'text-gray-500'}`}>
                  {t('reports.fuelBreakdown.vehiclesCount', { count: fuel.vehicles })}
                </span>
              </div>
            ...
            <p className={`text-xs pt-0.5 ${dark ? 'text-slate-500' : 'text-gray-500'}`}>
              {t('reports.fuelBreakdown.consumptionLabel', { value: fuel.consumption })}
            </p>
          </div>
        ))}
      </div>
