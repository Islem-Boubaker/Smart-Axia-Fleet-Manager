  ...
  <section className="space-y-3">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
      <Card dark={dark} padding="sm">
        <p className={dark ? 'text-xs text-slate-400' : 'text-xs text-slate-500'}>Jobs completed</p>
        <p className={`mt-1 text-2xl font-semibold ${dark ? 'text-white' : 'text-slate-900'}`}>
          {formatNumber(maintenanceStats.jobsCompleted)}
        </p>
      </Card>
      <Card dark={dark} padding="sm">
        <p className={dark ? 'text-xs text-slate-400' : 'text-xs text-slate-500'}>Total cost</p>
        <p className={`mt-1 text-2xl font-semibold ${dark ? 'text-white' : 'text-slate-900'}`}>
          {formatCurrency(maintenanceStats.totalCost)}
        </p>
      </Card>
      <Card dark={dark} padding="sm">
        <p className={dark ? 'text-xs text-slate-400' : 'text-xs text-slate-500'}>Avg downtime per job</p>
        <p className={`mt-1 text-2xl font-semibold ${dark ? 'text-white' : 'text-slate-900'}`}>
          {maintenanceStats.avgDowntime.toFixed(1)} days
        </p>
      </Card>
    </div>
          ...
          >
            <div>
              ...
              <p className={dark ? 'text-xs text-slate-500' : 'text-xs text-slate-500'}>
                {row.jobs} jobs{row.types.size ? ` · ${Array.from(row.types).join(', ')}` : ''}
              </p>
            </div>
