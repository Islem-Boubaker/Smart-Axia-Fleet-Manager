  ...
  <section className="space-y-3">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
      <Card dark={dark} padding="sm">
        <p className={dark ? 'text-xs text-slate-400' : 'text-xs text-slate-500'}>Total trips</p>
        <p className={`mt-1 text-2xl font-semibold ${dark ? 'text-white' : 'text-slate-900'}`}>
          {formatNumber(analytics.totalTrips)}
        </p>
      </Card>
      <Card dark={dark} padding="sm">
        <p className={dark ? 'text-xs text-slate-400' : 'text-xs text-slate-500'}>Total km driven</p>
        <p className={`mt-1 text-2xl font-semibold ${dark ? 'text-white' : 'text-slate-900'}`}>
          {formatNumber(analytics.totalKm)}
        </p>
      </Card>
      <Card dark={dark} padding="sm">
        <p className={dark ? 'text-xs text-slate-400' : 'text-xs text-slate-500'}>On-time rate</p>
        <p className={`mt-1 text-2xl font-semibold ${dark ? 'text-white' : 'text-slate-900'}`}>
          {analytics.onTimeRate.toFixed(1)}%
        </p>
      </Card>
      <Card dark={dark} padding="sm">
        <p className={dark ? 'text-xs text-slate-400' : 'text-xs text-slate-500'}>Cancellation rate</p>
